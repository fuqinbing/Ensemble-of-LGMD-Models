%LGMD1_LGMD2_CascadeNetwork_Dual_Channel(img1, img2, t, parameter)

function [parameter] = LGMD1_LGMD2_CascadeNetwork_Dual_Channel(img1, img2, t, parameter)
    ...
    parameter.points(cur_t).LGMD1_Photoreceptor = ...
        Highpass(img1, img2, parameter.points(pre_t).LGMD1_Photoreceptor, parameter.delay_hp);
    tmp_ffi = sum(sum(abs(parameter.points(cur_t).LGMD1_Photoreceptor)));
    ...
    Blurred = imfilter(parameter.points(cur_t).LGMD1_Photoreceptor, Gauss_kernel, 'same');
    
    parameter.points(cur_t).LGMD1_ONs = Halfwave_ON(Blurred, parameter.points(pre_t).LGMD1_ONs, parameter.dc, parameter.Clip_point);
    parameter.points(cur_t).LGMD1_OFFs = Halfwave_OFF(Blurred, parameter.points(pre_t).LGMD1_OFFs, parameter.dc, parameter.Clip_point);
    ...
    parameter.points(cur_t).LGMD1_Summation = ...
        PolarityChannelSummation(S_on, S_off, parameter.ON_exp, parameter.OFF_exp);
    
    LGMD2_Photoreceptor = Highpass2(parameter.points(pre_t).LGMD1_Summation, parameter.points(cur_t).LGMD1_Summation);
    
    parameter.points(cur_t).LGMD2_ONs = Halfwave_ON(LGMD2_Photoreceptor, ...
        parameter.points(pre_t).LGMD2_ONs, parameter.dc, parameter.Clip_point);
    parameter.points(cur_t).LGMD2_OFFs = Halfwave_OFF(LGMD2_Photoreceptor, ...
        parameter.points(pre_t).LGMD2_OFFs, parameter.dc, parameter.Clip_point);
    ...
    LGMD2_Summation = PolarityChannelSummation(S_on, S_off,parameter.ON_exp, parameter.OFF_exp);
 
    LGMD1_Compressed = NormalizeContrast(parameter. Kernel_Contrast, LGMD2_Summation, parameter. Ccn);
